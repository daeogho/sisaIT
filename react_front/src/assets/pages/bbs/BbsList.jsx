...
function BbsList() {
        ...
        function getBbsList(page){
                ...
                axios.get('http://43.202.150.110:9988/bbs/bbsList' + url)
                .then((response)=>{
                        ...
                        setList([]); // 기존 레코드 초기화                   
                        response.data.bbsList.map((record)=>{
                                setList((previous)=>{
                ...
                <div className="container">
                        ...
                        </div>
                        {
                                list.map((record, idx)=>{
                                        return(
                                                ...
                                                <div key={idx} className="row" style={{borderBottom:'1px solid #ccc'}} key={record.id}>
                                                        <div className="col-sm-1 p-3 ">{record.id}</div>
                                                        <div className="col-sm-7 p-3 "><Link to={`/bbs/bbsDetail/${record.id}`}>{record.subject}</Link></div>
                        ...
                        <ul className="pagination justify-content-center" style={{margin:'50px 0'}}>
                                {/*이전페이지*/}
                                {
                                        (pages.nowPage>1)
                                        &&(  
                                                <li className="page-item"><Link className="page-link" onClick={()=>getBbsList(pages.nowPage-1)}>Prev</Link></li>
                                        )
                                }
                                        ...
                                        pagenum.map((p, i)=>{
                                                ...
                                                return(
                                                        <li key={i} className={activeStyle}>
                                                                <Link className="page-link" onClick={()=>getBbsList(p)}>{p}</Link>
                                                        </li>
                                                )
                                        })
                                }  
                                ...
                                {/*다음페이지*/}
                                {
                                        ...
                                        &&(<li className="page-item"><Link className="page-link" onClick={()=>getBbsList(pages.nowPage+1)}>Next</Link></li>)
                                }
                        ...
                        <div style={{padding:'30px 0'}}>
                                ...
                                <input type="text" name="searchWord" id="searchWord"
                                        ...
                                        />
                                <input type="button" value="검색" onClick={()=>getBbsList(1)}/>
                        </div>

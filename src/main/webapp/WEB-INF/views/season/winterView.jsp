	            ...
	            if (!data || data.length === 0) {
	                ...
	                data.forEach(function(sr) {
	                    replyHtml += `
	                            ...
	                            <div style="display:flex; flex-direction: row; justify-content:space-between;">
                                	...
                                	\${logId !== "" && logId === sr.userID ? `
                                            <div style="display:flex; flex-direction: row; justify-content:center;">
                                                <button type="button" class="diphylleia-regular-button" onclick="replyEdit('\${sr.comment_no}', '\${sr.context}')" style="background-color:#9BD1DE; border: 0px; border-radius:10px; padding:5px;">수정</button>
	                        ...
	                        <hr style="width: 100%; border:none; border-top: 5px solid #9BD1DE; margin: 0;">
	                    `;
	                });
	            }
	        ...
	        <form id="editForm-\${no}" style="margin-top:10px;">
	            <div style="display:flex; flex-direction: column;">
	                <textarea id="t\${no}" name="context" 
	                    ...
	                    overflow: hidden; box-sizing: border-box; border: 2px solid #9BD1DE;
	                ...
	                <div style="margin-top:5px; text-align:right;">
	                    <button type="button" class="diphylleia-regular-button" onclick="replyEditOk(\${no})" 
	                        style="background-color:#9BD1DE; border: 0px; border-radius:10px; padding:5px;">수정완료</button>
...
<div style="margin: 0 auto; width:100%; max-width:1000px; text-align:center;">
	<div style="padding:60px;">
		...
		<p style="color:#3e8697; font-weight:bold; font-size:20px;">주소: ${ss.addrtitle}</p>
...
<!-- 게시글 올린 이 프로필 -->
<div style="background-color:#E0F5F4;">
	<hr style="border:none; border-top: 5px solid #9BD1DE">
	<div style="display:flex; flex-direction: row; justify-content:center;">
...
<div style="display:flex; flex-direction: row; justify-content:center;">
	<button class="diphylleia-regular-button"
		...
		style="background-color:#3e8697; border: 0px; border-radius:10px; padding:5px; margin-right: 10px;">목록</button>
	<c:if test="${not empty logId && logId eq ss.userID}">
		<button class="diphylleia-regular-button" onclick="location.href='${ctx}/season/seasonEdit?no=${ss.no}'" style="background-color:#3e8697; border: 0px; border-radius:10px; padding:5px; margin-right: 10px;">수정</button>
		...
		<div style="display:flex; flex-direction: column;">
			<input type="hidden" name="no" value="${ss.no}">
   			...
   			<c:if test="${logId!=null && logId!=''}">
				<p style="color:#9BD1DE; margin: 0; font-weight: bold;">작성 이름:${logId }</p>
			</c:if>	
			<div>
				...
				<button class="diphylleia-regular-button" onclick="replyWrite()" style="background-color:#3e8697; border: 0px; border-radius:10px; padding:5px;">등록</button>
			</div>
		</div>
	</form>
</div>
...
<div style="background-color:#E0F5F4; display:flex; flex-direction: column; align-items: center;">
	<hr style="width: 100%; border:none; border-top: 5px solid #9BD1DE; margin: 0;">

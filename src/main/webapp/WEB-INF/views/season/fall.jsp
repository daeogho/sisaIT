...
<div class="main-top"  >
	...
	<div class="main-img" style="position:relative; width:100%;">
		<img src="../resources/image/fallmain.png" style="height:450px; width:100%; object-fit: cover;"/>
		<img src="${ctx}/resources/image/Flogo.png" style="position:absolute; top:30px; right:50%; z-index:10; width:120px"/>
...
<div style=" justify-content:center; align-items:center; height:100px; display: flex; margin:100px auto 20px;  max-width:1400px; ">
	...
	<h2 style="line-height:100px;  font-family:Gowun Batang; font-weight: bold;  color:#456e2a">돌담 : 제주의 가을</h2>
...
<div style="width:1400px; display:flex; justify-content:center; margin:50px auto; ">
		<form method="get" action="${ctx}/season/list?season=fall" onsubmit="return searchCheck()"  >
...
<div style="display:flex; flex-direction: row; margin: 0 auto;
	...
	<c:forEach var="ss" items="${list }">
		<div class="card" style="position:relative; border:none; width:300px; height:250px; margin:10px 10px; flex-wrap:wrap;">
				...
				<a href="${ctx }/season/fallView?no=${ss.no}&nowPage=${pVO.nowPage }<c:if test="${pVO.searchWord!=null and pVO.searchWord!='' }">&searchKey=${pVO.searchKey }&searchWord=${pVO.searchWord }</c:if>" style="text-decoration:none; color:black;">
...
<ul class="pagination justify-content-center">
    <c:if test="${pVO.nowPage==1 }">
    	<li class="page-item"><a class="page-link" href="javascript:void(0);" style="color:#E5AF4F !important;">Prev</a></li>
    </c:if>
    <c:if test="${pVO.nowPage>1 }">
    	<li class="page-item"><a class="page-link" href='${pageContext.request.contextPath }/season/list?fall=spring&nowPage=${pVO.nowPage-1}<c:if test="${pVO.searchWord!=null and pVO.searchWord!='' }">&searchKey=${pVO.searchKey }&searchWord=${pVO.searchWord }</c:if>' style="color:#E5AF4F !important;">Prev</a></li>
    </c:if>
	<!-- 이전페이지 -->
    	...
    	<c:if test="${pg<=pVO.totalPage }">
    		...
    		<c:if test="${pg!=pVO.nowPage }">
    			<li class="page-item">
    				...
    				<a class="page-link" href='${pageContext.request.contextPath }/season/list?season=fall&nowPage=${pg}<c:if test="${pVO.searchWord!=null and pVO.searchWord!='' }">&searchKey=${pVO.searchKey }&searchWord=${pVO.searchWord }</c:if>' style="color:#E5AF4F !important;">${pg }</a>
    			</li>
		</c:if>
	</c:forEach>
	<!-- 다음 페이지 -->
	<c:if test="${pVO.nowPage<pVO.totalPage }">
    	<li class="page-item"><a class="page-link" href='${pageContext.request.contextPath }/season/list?season=fall&nowPage=${pVO.nowPage+1 }<c:if test="${pVO.searchWord!=null and pVO.searchWord!='' }">&searchKey=${pVO.searchKey }&searchWord=${pVO.searchWord }</c:if>' style="color:#E5AF4F !important;">Next</a></li>
    </c:if>
    <c:if test="${pVO.nowPage==pVO.totalPage }">
    	<li class="page-item"><a class="page-link" href="javascript:void(0);" style="color:#E5AF4F !important;">Next</a></li>
    </c:if>
</ul>
